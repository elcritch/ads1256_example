...
/* Command definition£º TTable 24. Command Definitions --- ADS1256 datasheet
 * Page 34 */
enum {
...
int main() {
  ...
  */
  ...
  //}
  Write_DAC8552(0x30,
                Voltage_Convert(5.0, 2.5)); // Write channel A buffer (0x30)
  Write_DAC8552(0x34,
                Voltage_Convert(5.0, 3.1)); // Write channel B buffer (0x34)
  while (1) {
    ...
    for (i = 0; i < ch_num; i++) {
      ...
      printf("%d=%02X%02X%02X, %8ld", (int)i, (int)buf[0], (int)buf[1],
             (int)buf[2], (long)adc[i]);

      iTemp = volt[i]; /* uV  */
      if (iTemp < 0) {
        iTemp = -iTemp;
        printf(" (-%ld.%03ld %03ld V) \r\n", iTemp / 1000000,
               (iTemp % 1000000) / 1000, iTemp % 1000);
      } else {
        printf(" ( %ld.%03ld %03ld V) \r\n", iTemp / 1000000,
               (iTemp % 1000000) / 1000, iTemp % 1000);
      }
